...
class TestFCUFoodGrade(unittest.TestCase):
    def setUp(self):
        ...
        with self.app.app_context():
            db.create_all()
            # 建立基本測試資料
            self.user = User.create(username="測試生", email="d1112223@o365.fcu.edu.tw", password="password123")
    ...
    def test_auth_flow(self):
        ...
        response = self.client.post('/auth/register', data={
            'username': '新同學',
            'email': 'd1234567@o365.fcu.edu.tw',
            'password': 'password123',
            'confirm_password': 'password123'
        }, follow_redirects=True)
    ...
    def test_review_and_rating_averages(self):
        ...
        self.client.post('/auth/login', data={
            'email': 'd1112223@o365.fcu.edu.tw',
            'password': 'password123'
        })
    ...
    def test_create_restaurant_flow(self):
        ...
        self.assertIn('地圖定位店家'.encode('utf-8'), response.data)

    def test_map_and_savior_pages(self):
        """
        7. 測試美食地圖與選擇困難救星頁面
        """
        # 美食地圖
        ...
        self.assertEqual(response.status_code, 200)
        self.assertIn('逢甲美食地圖'.encode('utf-8'), response.data)

        # 選擇困難救星
        response = self.client.get('/savior')
        self.assertEqual(response.status_code, 200)
        self.assertIn('選擇困難救星'.encode('utf-8'), response.data)
