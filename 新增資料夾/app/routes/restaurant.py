...
def food_map():
    ...
    return render_template('restaurant/map.html', restaurants=restaurants)

@restaurant_bp.route('/savior')
def savior():
    """
    選擇困難救星頁面
    """
    restaurants = Restaurant.query.all()
    all_tags = Tag.get_all()
    return render_template('restaurant/savior.html', restaurants=restaurants, landmarks=LANDMARKS, tags=all_tags)
